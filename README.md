...
| **Database** | PostgreSQL 16 + Prisma (37 models) |
...
| Security | JWT + RBAC, TOTP MFA, bcrypt, audit logs, PHI masking |
...
Compliance: [docs/SUBMISSION_AUDIT.md](docs/SUBMISSION_AUDIT.md) (~95%) · [docs/ASSIGNMENT_COMPLIANCE.md](docs/ASSIGNMENT_COMPLIANCE.md) · [docs/FINAL_SUBMISSION_REVIEW.md](docs/FINAL_SUBMISSION_REVIEW.md)
...
| Domain | Capabilities |
|--------|-------------|
| **Auth** | Register, login, TOTP MFA, refresh rotation, logout, profile (`/auth/*`) |
...
| ORM | Prisma 6 | Type-safe migrations, 37 models |
...
| Infra | Docker, Kubernetes, **Terraform (AWS)** | Production deployment path |
...
├── src/
│   ├── modules/
│   │   ├── auth/                 # Register, login, MFA, refresh, profile
...
│   ├── app.module.ts
│   └── main.ts
├── prisma/                       # Schema (37 models), migrations, seed
...
| `MFA_ENABLED` | Enable TOTP MFA APIs | `true` |
| `MFA_ENCRYPTION_KEY` | AES-256-GCM key for TOTP secrets | Falls back to JWT secret |
...
```bash
# Unit tests (51 tests / 17 suites — booking, MFA, idempotency, guards)
npm test

# Unit tests with coverage (17% threshold; ~20% current)
npm run test:cov
...
npm run loadtest:benchmark   # Auth + search + booking + consultation SLOs
```

Additional scenarios in `loadtests/scenarios/`: `stress.js`, `spike.js`, `soak.js`.

Report: [docs/performance/BENCHMARK_REPORT.md](docs/performance/BENCHMARK_REPORT.md).

Workloads authenticate via real `POST /auth/login` and exercise `/doctors`, booking, and consultations — see `loadtests/workloads.js` and `loadtests/scenarios/benchmark.js`.
...
sequenceDiagram
    ...
    alt MFA enrolled
        API-->>C: mfaRequired + mfaToken (5m)
        C->>API: POST /auth/mfa/verify + OTP
    ...
    API->>DB: JwtStrategy reloads user (ACTIVE check)
```

| Endpoint | Auth | Description |
|----------|------|-------------|
| `POST /auth/register` | Public | Patient registration |
| `POST /auth/login` | Public | Tokens **or** MFA challenge |
| `POST /auth/mfa/enable` | Bearer | Begin TOTP enrollment (QR + recovery codes) |
| `POST /auth/mfa/verify-setup` | Bearer | Confirm OTP → enable MFA |
| `POST /auth/mfa/verify` | Public | Complete MFA challenge → tokens |
| `POST /auth/mfa/disable` | Bearer | Disable MFA (password + OTP) |
| `POST /auth/refresh` | Public | Rotate refresh token |
| `POST /auth/logout` | Bearer | Revoke refresh token |
| `GET/PATCH /auth/me` | Bearer | Profile read/update |
| `GET /auth/mfa/status` | Bearer | MFA enrollment status |

Set `MFA_ENABLED=true` and optional `MFA_ENCRYPTION_KEY` (AES-256-GCM for TOTP secrets).

**Guards:** Global `JwtAuthGuard` → `RolesGuard` → service-level ownership.

Implementation: [`auth.service.ts`](src/modules/auth/application/services/auth.service.ts) · [`mfa.service.ts`](src/modules/auth/application/services/mfa.service.ts) · [ADR-007](docs/adr/007-jwt-authentication.md)
...
**51 HTTP endpoints** across 10 controllers. Full specification: [docs/openapi.yaml](docs/openapi.yaml).
...
| MFA | TOTP (otplib) + AES-256-GCM encrypted secrets + recovery codes |
...
Provisions VPC, RDS PostgreSQL, ElastiCache Redis, ALB, ECS Fargate (or optional EKS), Secrets Manager, CloudWatch alarms. Full guide: [infra/terraform/README.md](infra/terraform/README.md).
...
| Prisma ORM | [002](docs/adr/002-prisma-orm.md) | Type-safe migrations for 37 models |
...
- Per-route rate limits on `/auth/login` and `/auth/mfa/verify`
...
| [docs/SUBMISSION_AUDIT.md](docs/SUBMISSION_AUDIT.md) | Pre-submission audit (~95%) |
| [docs/FINAL_SUBMISSION_REVIEW.md](docs/FINAL_SUBMISSION_REVIEW.md) | Final review, demo screenplay, checklist |
| [SECURITY.md](SECURITY.md) | Threat model and controls |
| [CONTRIBUTING.md](CONTRIBUTING.md) | Development conventions |
| [CHANGELOG.md](CHANGELOG.md) | Version history |
| [LICENSE](LICENSE) | UNLICENSED (portfolio / evaluation) |

---

## License

UNLICENSED — portfolio / evaluation project. See [LICENSE](LICENSE).
